...
} from "./detail_pesanan/detail_pesanan_fields";
import DetailDesainNailArt from "./detail_pesanan/detail_desain_nail_art";
...
export default function ModalDetailPesanan({
  ...
  data,
  variant = "masuk",
}: PropsModalDetailPesanan) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6"
    >
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-lg bg-[#ffecf2] shadow-xl"
        ...
        <div className="space-y-6 px-5 py-5">
          ...
          <div className="rounded-lg border border-[#dd98ad] shadow-soft-text bg-[#ffecf2] p-4">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <InputReadonly label="Nominal Pembayaran" value={data?.nominalPembayaran} />
              <InputReadonly label="Harga Final" value={data?.hargaFinal} />
              <InputReadonly label="Status Pembayaran" value={data?.statusPembayaran} />
              <InputReadonly label="Tanggal Pembayaran" value={data?.tanggalPembayaran} />
              
              {(variant === "aktif" || variant === "riwayat") && (
                <InputReadonly label="Tanggal Verifikasi" value={data?.tanggalVerifikasi} />
              )}
